		...
		unsigned int index, unsigned int addr, u32 val)
{
	...
	printk("csi5_phy_write, csi5_fops.c:52\n");
	...
	u32 csi_port)
{
	...
	dev_info(csi->dev, "%s: stream_id=%u, csi_port=%u, csi5_fops.c:94\n",
		__func__, stream_id, csi_port);
	/* Open NVCSI stream */
	...
	u32 csi_port, int csi_lanes)
{
	...
	int phy_mode_from_s_data=s_data->sensor_props.sensor_modes->signal_properties.phy_mode;
	bool is_cphy = ( phy_mode_from_s_data == 1 );

	dev_dbg(csi->dev, "%s: stream_id=%u, csi_port=%u\n",
		__func__, stream_id, csi_port);
	dev_info(csi->dev, "%s: stream_id=%u, csi_port=%u,phy_mode = %d, csi5_fops.c:147\n",
		__func__, stream_id, csi_port,s_data->sensor_props.sensor_modes->signal_properties.phy_mode);
	dev_info(csi->dev, "csi_lanes = %d\n",csi_lanes);
	/* Brick config */
...
static int csi5_mipi_cal(struct tegra_csi_channel *chan)
{
	...
	dev_dbg(csi->dev, "%s\n", __func__);
	dev_info(csi->dev, "csi5_mipi_cal,phy_mode = %d, csi5_fops.c\n",phy_mode);
	lanes = 0;
	num_ports = 0;
	csi_port = 0;
