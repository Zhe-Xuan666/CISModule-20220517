...
/*
*更動部分,主要為comment掉不需要之function,實現使agx xavier不做i2c功能目的: 
* EEPROM //in our case,we don't have EEPROM.
* FUSEID //Filesystem in Userspace.
* OTP //One-Time Programmable.
* debugfs //usage is debugging.
* i2c write and read func. //our case is setting by external device.
*/
...
static const u32 ctrl_cid_list[] = {
	...
	/*
	TEGRA_CAMERA_CID_EEPROM_DATA, //Reads the module’s EEPROM data.
	TEGRA_CAMERA_CID_OTP_DATA, //Reads the module’s One-Time Programmable (OTP) memory data.
...
//Don't need to setup EEPROM.OTP.FUSE ID
/*
...
{
	struct ov5693 *priv = (struct ov5693 *)tegracam_get_privdata(tc_dev);
	//remove err status detect func & i2c table write.
	/*
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
...
static struct camera_common_sensor_ops ov5693_common_ops = {
	...
	//.read_reg = ov5693_read_reg, //already remove this func.
			...
			const struct i2c_device_id *id)
{
	...
	//modified printout
	dev_info(dev, "probing v4l2 sensor without i2c. //xuan modified\n");
	...
	*/
	...
	if (err) {
		dev_err(dev, "tegra camera subdev registration failed //xuan modified\n");
		return err;
	}

	//Don't need to create debugfs 
	/*
